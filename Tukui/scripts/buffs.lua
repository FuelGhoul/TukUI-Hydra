...
for i = 1, 3 do
	...
	_G["TempEnchant"..i.."Duration"]:SetPoint("BOTTOM", 0, TukuiDB.Scale(-15))
...
local function StyleBuffs(buttonName, index, debuff)
	...
	if icon and not _G[buttonName..index.."Panel"] then
		...
		duration:SetPoint("BOTTOM", 0, TukuiDB.Scale(-15))
	...
	for index=1, BUFF_ACTUAL_DISPLAY do
		...
		if ( buff.consolidated ) then
			...
			elseif ( index == 1 ) then
				...
				buff:SetPoint("RIGHT", previousBuff, "LEFT", TukuiDB.Scale(-5), 0)
			end
			previousBuff = buff
		end		
	end
...
local function UpdateDebuffAnchors(buttonName, index) -- local function UpdateDebuffAnchors(buttonName, index)
...
local function UpdateTime(button)
	...
	if SHOW_BUFF_DURATIONS == "1" then
		duration:SetTextColor(unpack(TukuiCF.buffs.color))
	end
end
...
--[[local HyBuff = CreateFrame("Frame")
local function UpdateHyBuffs(self)
	return UpdateBuffAnchors()
end
HyBuff:SetScript("OnUpdate", UpdateHyBuffs)
UpdateHyBuffs(HyBuff)]]