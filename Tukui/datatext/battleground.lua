		...
		if int < 0 then
			...
			for i=1, numScores do
				...
				if healingDone > damageDone then
					dmgtxt = (tukuilocal.datatext_healing..hexa..healingDone..hexb)
				else
					dmgtxt = (tukuilocal.datatext_damage..hexa..damageDone..hexb)
				end
				if ( name ) then
					if ( name == TukuiDB.myname ) then
						Text2:SetText(tukuilocal.datatext_honor..hexa..format('%d', honorGained)..hexb)
						Text1:SetText(dmgtxt)
						Text3:SetText(tukuilocal.datatext_killingblows..hexa..killingBlows..hexb)
					end   
				end
			end 
			int  = 1
