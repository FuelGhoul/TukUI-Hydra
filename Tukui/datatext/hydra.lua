...
-- Justice Points
if not TukuiCF["datatext"].justice == nil or TukuiCF["datatext"].justice > 0 then
   local Stat = CreateFrame("Frame")

   local Text  = TukuiInfoLeft:CreateFontString(nil, "OVERLAY")
   Text:SetFont(TukuiCF.media.font, TukuiCF["datatext"].fontsize)
   TukuiDB.PP(TukuiCF["datatext"].justice, Text)
   ...
   local function Update(self, t)
     int = int - t
     if int < 0 then
	 local localized_label, amount, icon_file_name = GetCurrencyInfo(395)
	 
       Text:SetText(hexa.."Justice Points"..hexb..": "..amount)
       int = 1
     end     
   end
